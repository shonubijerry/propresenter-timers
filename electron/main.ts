...
function initializeLogging(): void {
  try {
    ...
    log(`Failed to initialize logging: ${JSON.stringify(error)}`)
  }
}
...
function createWindow(): void {
  ...
  if (isDev) {
    ...
    startLocalServer()
      .then((port) => {
        log(`Loading app from local server on port ${port}`)
      ...
      .catch((err) => {
        ...
        log(`Fallback: loading file directly from ${fallbackPath}`)
